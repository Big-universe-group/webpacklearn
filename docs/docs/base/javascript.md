...
有人可能会问，js 资源 Webpack 不能已经处理了吗，为什么我们还要处理呢？其还有什么问题吗？

+ Webpack 对 js 处理是有限的，只能编译 js 中 ES 模块化语法，不能编译其他语法（如箭头函数），导致 js 不能在 IE 等浏览器运行，所以我们希望做一些兼容性处理。
+ 团队对代码格式是有严格要求的，我们不能由肉眼去检测代码格式，需要使用专业的工具来检测。

针对上面的两个问题，我们使用下面的两个方式来解决：

- 针对 js 兼容性处理，我们使用 Babel 来完成JS代码的转换，从ES6转为ES5
...
module.exports = {
  extends: ["react-app"],
  rules: {
    // 我们的规则会覆盖掉react-app的规则，所以想要修改规则直接改就是了
    eqeqeq: ["warn", "smart"],
  },
};
```
...
简单理解：就是一组预先配置的Babel 插件, 扩展 Babel 功能
...
具体参考：[官网地址](https://webpack.docschina.org/loaders/babel-loader/).
...
module.exports = {
  ...
  module: {
    rules: [
      ...
      // babel使用，其他option在babel.config.js中配置
      {
        test: /\.js$/,
